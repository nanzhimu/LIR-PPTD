...
from .runner import (
    R1C_PLANNED_METHOD_RUNS,
    UPSTREAM_E1_RAW_SHA256,
    UPSTREAM_E2_RAW_SHA256,
    UPSTREAM_E2_SUMMARY_SHA256,
    UPSTREAM_PHASE_R1_START_BINDING_SHA256,
    UPSTREAM_R1B_START_BINDING_SHA256,
)


class CombinedStatisticsV2Error(RuntimeError):
    pass
...
def _load_complete_records(
    root: Path,
) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any], dict[str, Any], str]:
    ...
    r1c_summary_path = root / "results/summary/phase_r1c_formal_summary.json"
    for path in (e1_summary_path, e2_summary_path, r1c_summary_path):
        if not path.is_file():
            raise CombinedStatisticsV2Error(f"COMBINED_FORMAL_SUMMARY_MISSING:{path.name}")

    e1_summary = json.loads(e1_summary_path.read_text(encoding="utf-8"))
    e2_summary = json.loads(e2_summary_path.read_text(encoding="utf-8"))
    r1c_summary = json.loads(r1c_summary_path.read_text(encoding="utf-8"))

    if e1_summary.get("start_binding_sha256") != UPSTREAM_PHASE_R1_START_BINDING_SHA256:
        raise CombinedStatisticsV2Error("E1_BINDING_MISMATCH")
    if e1_summary.get("raw_sha256") != UPSTREAM_E1_RAW_SHA256:
        raise CombinedStatisticsV2Error("E1_RAW_SHA_IN_SUMMARY_MISMATCH")
    if int(e1_summary.get("committed_method_runs", -1)) != 1686:
        raise CombinedStatisticsV2Error("E1_COMPLETION_COUNT_MISMATCH")

    if e2_summary.get("r1b_start_binding_sha256") != UPSTREAM_R1B_START_BINDING_SHA256:
        raise CombinedStatisticsV2Error("E2_BINDING_MISMATCH")
    if e2_summary.get("raw_sha256") != UPSTREAM_E2_RAW_SHA256:
        raise CombinedStatisticsV2Error("E2_RAW_SHA_IN_SUMMARY_MISMATCH")
    if _sha256(e2_summary_path) != UPSTREAM_E2_SUMMARY_SHA256:
        raise CombinedStatisticsV2Error("E2_SUMMARY_SHA_MISMATCH")
    if int(e2_summary.get("committed_method_runs", -1)) != 1080:
        raise CombinedStatisticsV2Error("E2_COMPLETION_COUNT_MISMATCH")

    if r1c_summary.get("status") != "complete":
        raise CombinedStatisticsV2Error("R1C_NOT_COMPLETE")
    if int(r1c_summary.get("committed_method_runs", -1)) != R1C_PLANNED_METHOD_RUNS:
        raise CombinedStatisticsV2Error("R1C_COMPLETION_COUNT_MISMATCH")

    e1_raw = root / str(e1_summary["raw_path"])
    e2_raw = root / str(e2_summary["raw_path"])
    r1c_raw = root / str(r1c_summary["raw_path"])
    if _sha256(e1_raw) != str(e1_summary["raw_sha256"]):
        raise CombinedStatisticsV2Error("E1_RAW_HASH_MISMATCH")
    if _sha256(e2_raw) != str(e2_summary["raw_sha256"]):
        raise CombinedStatisticsV2Error("E2_RAW_HASH_MISMATCH")
    if _sha256(r1c_raw) != str(r1c_summary["raw_sha256"]):
        raise CombinedStatisticsV2Error("R1C_RAW_HASH_MISMATCH")

    e1_records = _read_jsonl(e1_raw)
    e2_records = _read_jsonl(e2_raw)
    r1c_records = _read_jsonl(r1c_raw)

    if len(e1_records) != 1686 or len(e2_records) != 1080 or len(r1c_records) != R1C_PLANNED_METHOD_RUNS:
        raise CombinedStatisticsV2Error("COMBINED_RAW_COUNT_MISMATCH")
    if any(r.get("stage") != "E1" for r in e1_records):
        raise CombinedStatisticsV2Error("E1_STAGE_CONTAMINATION")
    if any(r.get("stage") != "E2" for r in e2_records):
        raise CombinedStatisticsV2Error("E2_STAGE_CONTAMINATION")
    if any(r.get("stage") not in {"E3", "E4"} for r in r1c_records):
        raise CombinedStatisticsV2Error("R1C_STAGE_CONTAMINATION")

    run_ids = [str(r.get("run_id")) for r in e1_records + e2_records + r1c_records]
    if len(run_ids) != 3186 or len(set(run_ids)) != 3186:
        raise CombinedStatisticsV2Error("COMBINED_RUN_ID_UNIQUENESS_FAILURE")

    if any(r.get("r1b_start_binding_sha256") != UPSTREAM_R1B_START_BINDING_SHA256 for r in e2_records):
        raise CombinedStatisticsV2Error("E2_RECORD_BINDING_MISMATCH")
    r1c_binding = str(r1c_summary["r1c_start_binding_sha256"])
    if any(r.get("r1c_start_binding_sha256") != r1c_binding for r in r1c_records):
        raise CombinedStatisticsV2Error("R1C_RECORD_BINDING_MISMATCH")

    # Mechanical v2 adaptation of the already-prebound R1B analyzer:
    # same hypotheses/statistics; only the physical storage is split E1 | E2 | E3/E4.
    # The seed formula is fixed before R1C outcomes and incorporates the three frozen bindings.
    seed_root = hashlib.sha256(
        (
            "phase-r1-combined-analysis-v2|"
            + UPSTREAM_PHASE_R1_START_BINDING_SHA256
            + "|"
            + UPSTREAM_R1B_START_BINDING_SHA256
            + "|"
    ...
    return e1_records + e2_records + r1c_records, e1_summary, e2_summary, r1c_summary, seed_root


def analyze(root: Path) -> dict[str, Any]:
    records, e1_summary, e2_summary, r1c_summary, seed_root = _load_complete_records(root)
        ...
        for (dataset, attack_condition, rho, order_seed_text), members in sorted(groups.items()):
            ...
            for endpoint_field, endpoint_label in (
                ...
                if len(deltas) + failed_or_missing > planned:
                    raise CombinedStatisticsV2Error("PAIR_COUNT_EXCEEDS_PLAN")
                ...
                contrasts.append(
                    {
                        ...
                        "inference_status": (
                            "complete" if len(deltas) == planned
                            else "degraded_by_failures_or_missing_pairs"
                        ),
    ...
    return {
        "schema_version": "1.0",
        "phase": "Phase-R1+R1B+R1C",
        "analysis_status": "post_phase11_extension_prebound_before_R1C_outcomes",
        ...
        "r1c_start_binding_sha256": r1c_summary["r1c_start_binding_sha256"],
        "r1c_raw_sha256": r1c_summary["raw_sha256"],
        "e1_formal_summary_sha256": _sha256(root / "results/summary/phase_r1_formal_summary.json"),
        "e2_formal_summary_sha256": _sha256(root / "results/summary/phase_r1b_formal_summary.json"),
        "r1c_formal_summary_sha256": _sha256(root / "results/summary/phase_r1c_formal_summary.json"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Phase-R1 + R1B + R1C prebound combined statistical analyzer"
    )
    ...
    if args.validate_only:
        ...
        print("R1_COMBINED_STATISTICAL_PLAN_V2_VALIDATE=PASS")
        print(f"BOOTSTRAP_RESAMPLES={result['bootstrap_resamples']}")
        print("TEST_DIRECTION=two_sided")
        print("STATISTICAL_CONTRACT_CHANGED=NO")
    ...
    if out.exists():
        raise CombinedStatisticsV2Error(f"REFUSING_TO_OVERWRITE:{out}")
